...
const App: React.FC = () => {
  ...
  // UX: skeleton leve ao abrir o dashboard (substitui o pré-loader)
  const [isDashboardBootLoading, setIsDashboardBootLoading] = useState(false);
  ...
  // Entrada suave do app (sem pré-loader)
  useEffect(() => {
    const t = window.setTimeout(() => setAppEntered(true), 10);
    return () => window.clearTimeout(t);
  }, []);

  // Skeleton leve quando o usuário entra no dashboard
  useEffect(() => {
    if (!isAuthed) return;
    if (view !== 'indicadores') return;
    setIsDashboardBootLoading(true);
    const t = window.setTimeout(() => setIsDashboardBootLoading(false), 550);
    return () => window.clearTimeout(t);
  }, [isAuthed, view]);
  ...
  // Gate de acesso: Login → Dashboard
  if (!isAuthed) return (
            ...
            {view === 'indicadores' && (
              <div className="p-10">
                {isDashboardBootLoading ? (
                  ...
                  <DashboardKPIs leads={state.leads} transactions={state.transactions} meta={state.profile.meta} />
                )}
              </div>
            )}
