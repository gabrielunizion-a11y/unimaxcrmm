...
// Compatibilidade: em conversas com suporte alguns tokens vêm como "token de autenticação"
// e o usuário pode salvar como PLACAFIPE_AUTH_TOKEN. Aceitamos ambos.
const PLACAFIPE_TOKEN = process.env.PLACAFIPE_TOKEN || process.env.PLACAFIPE_AUTH_TOKEN;
...
// Lê resposta do provedor com segurança.
// A API pode retornar HTML (ex.: páginas de erro/proxy) mesmo com status 200/4xx.
async function readJsonSafe(resp) {
  ...
  const err = new Error(`Resposta não-JSON do provedor (content-type: ${ct || 'desconhecido'}). Início: ${snippet}`);
  err.status = 502;
  throw err;
}
...
async function fetchPlacaFipe(placa) {
  if (!PLACAFIPE_TOKEN) {
    const err = new Error('Token PlacaFipe não configurado no servidor. Configure PLACAFIPE_TOKEN (ou PLACAFIPE_AUTH_TOKEN) em .env.local.');
    err.status = 500;
    throw err;
  }
  ...
  const data = await readJsonSafe(resp).catch((e) => {
    if (!e.status) e.status = resp.ok ? 502 : resp.status;
    throw e;
  });
...
// Garantir que toda rota /api não mapeada responda JSON (evita "Unexpected token '<'" no front)
app.use('/api', (req, res) => {
  res.status(404).json({ message: 'Endpoint /api não encontrado', path: req.originalUrl });
});
