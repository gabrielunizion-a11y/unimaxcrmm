...
export const LeadModal: React.FC<{
  ...
  const handlePlacaSearch = async (placa: string) => {
    ...
    if (p.length === 7) {
      ...
      try {
        ...
        // Se o backend/proxy não estiver ativo, pode retornar HTML (index/erro) e não JSON.
        ...
        if (!res.ok) {
          const msg = (data && typeof data === 'object')
            ? (data?.message || data?.error || 'Falha ao consultar placa')
            : 'API de placa indisponível. Rode "npm run dev:full" e confirme /api/health.';
          throw new Error(msg);
        }

        if (!data || typeof data !== 'object') {
          throw new Error('Resposta inválida da API. Rode "npm run dev:full" e confirme /api/health.');
        }
  ...
  const loadFipeHistory = async (codigoFipe: string, force: boolean = false) => {
    ...
    try {
      ...
      if (!res.ok || !data) {
        const msg = (data && typeof data === 'object')
          ? (data?.message || data?.error || 'Falha ao obter histórico FIPE')
          : 'API indisponível. Rode "npm run dev:full" e confirme /api/health.';
        throw new Error(msg);
      }
